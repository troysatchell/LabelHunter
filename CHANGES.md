...
report found 11 cases missing their expected verdict, 5 corpus-scope. TRO-538 (`routerVerdict`
vs `cascadeVerdict`) and TRO-535 (`OCR_CONFIDENCE_FLOOR` 60 to 50) landed first, unblocking
this ticket's C4, C6, and C7. This entry covers corrections C1 through C8 in order.
...
UnifrakturMaguntia. Neither case carried a `notes` field or cited a design document. The old
`expected` block predicted `REVIEW` / `LOW_MODEL_CONFIDENCE`. Two independent live runs
(`eval-report.json`, measured 2026-08-12T22:15:52.776Z; `benchmark-report.json`, measured
2026-08-12T22:30:58.027Z) both score the affected field `MATCH`, with no confidence drop.
Changed, for each case: `labelVerdict` to `PASS`; `reviewReason` removed (absent, matching
every other `PASS` case); the affected field's verdict to `MATCH`; its reason text restated
as the measured result. `description` also dropped its own falsified clause ("hard for a
model to read confidently") so it does not contradict the corrected `reason` text next to it.
...
edges, so a real model still read the label perfectly (diagnosis, section 3D). Per the case-20
precedent (an added blur, so the image becomes genuinely unreadable instead of the note being
restated to fit a weak image), `applyLowLight` (`scripts/golden/degrade.ts`) gains two new
optional parameters:

- `contrastFactor` — compresses the region's dynamic range toward mid-gray (128) before the
  existing exposure scale runs, simulating a sensor's noise floor crushing shadow and highlight
  detail, not just a proportionally darker copy of the same crisp edges.
- `noiseAmplitude` — a deterministic grain field, seeded with a fixed `mulberry32` generator,
  never `Math.random()`. Composited with the `"overlay"` blend mode, whose no-op point sits at
  exactly mid-gray, so `amplitude` alone controls how far the grain pushes a pixel.

Case-21's manifest entry chains this strengthened `low-light` step with an existing `blur`
step (sigma 1.6) — the same two-step pattern case-20 already uses. Both new parameters default
off (`contrastFactor: 1`, `noiseAmplitude: 0`), so every existing caller — case-22's own
`low-light` degradation, and every pre-existing test — stays byte-identical. Verified: a full
`pnpm golden:build` (all 32 cases) touches only `case-21-low-light-front-label.jpg`
(`git diff --stat golden-set/images/`).

Measured, real pixel statistics on the committed image, front region, grayscale (before —
this ticket's own edit — vs after):
...
(2.32:1 contrast ratio, gradient 73), confirming this measurement method matches the one the
diagnosis used. The image genuinely degrades now: a lower max gradient means an edge now
spreads over several pixels instead of one, and the compressed, noisier range is a real
dynamic-range change, not only a darker copy.

Case-21's own `expected` block — `labelVerdict`, `reviewReason`, every field verdict — is
UNCHANGED. C3 is a pixel correction, not a corpus-expectation edit. Live re-check
(`pnpm eval:check -- --live --case=case-21-low-light-front-label`, $0.004685): still scores
`PASS` (`labelVerdictCorrect: false`), extractor confidence 0.99 on both affected fields,
`image_quality.confidence: 0.95`. This is an honest result, not a partial fix. The diagnosis's
own finding S6 says `LOW_IMAGE_QUALITY`'s confidence-driven branch fired zero times across the
full 32-case corpus, because it depends entirely on the model's own self-reported confidence —
which this run confirms stays high even against a measurably degraded image. S6 is a separate,
already-documented code defect. It is not in this ticket's C1-C8 scope.

A stronger variant was tried and reverted (real spend: $0.014706, one extra live call):
`brightnessFactor` 0.55, `contrastFactor` 0.3, `noiseAmplitude` 38, blur sigma 2.4. It did flip
the label verdict to `REVIEW` — but for the wrong reason, and at a real cost. `applyBlur`
blurs the WHOLE image, not one region. The stronger blur also degraded the
`government_warning` block, breaking case-21's own "back label reads fine" premise. Measured:
all 5 field-level verdicts came back wrong on that run, not just the two front-label fields the
case is designed to test. Reverted to the modest values above, which keep `government_warning`
correct (confirmed by the targeted run in the paragraph above).

**C4 — case-23 / case-24, reviewReason corrected to the measured mechanism.** TRO-535
(2026-08-12, already merged) swept `OCR_CONFIDENCE_FLOOR` from 60 to 50. Confirmed in the
post-sweep artifacts before editing anything: `eval-report.json` (measured
2026-08-12T22:15:52.776Z) records both cases' `routerVerdict.warningChannel: "dual"` — the
second, OCR channel now participates, where the old floor discarded it. Both cases'
`routerVerdict.actualReviewReason` already reads `WARNING_MISMATCH`, not the manifest's old
`LOW_IMAGE_QUALITY`. Changed, for each case: `expected.reviewReason` from `LOW_IMAGE_QUALITY`
to `WARNING_MISMATCH`; the `governmentWarning` field's reason text, which named "extraction
confidence is low" — the mechanism the floor sweep replaced; the `notes` field, which now
records TRO-469/LH-021's original prediction on the record, next to TRO-535's and this
ticket's correction of it, rather than deleting the original claim.

Re-checked live, this ticket: case-23 router stage `labelVerdictCorrect: true`,
`reviewReasonCorrect: true` (`WARNING_MISMATCH`, haiku $0.004695 + resolver $0.009938). case-24
the same (haiku $0.004740 + resolver $0.010146). **Observed, not this ticket's job to fix:**
on both cases, the cascade end state (post-resolver) flips back to an incorrect `PASS` —
`resolverOutcome: "resolved"`. This is not new. TRO-538's own CHANGES.md entry already names
case-23 and case-24 among the cases the resolver flips from a correct router `REVIEW` to an
incorrect `PASS`.
...
orchestrator's scope ruling. `eval-report.json` and `benchmark-report.json` (both measured
2026-08-12T22:30:58.027Z or later) agree: the cascade end state (post-resolver) resolves both
cases to `FAIL`, matching the manifest's expectation exactly as written.

- case-28: `cascadeVerdict.actualLabelVerdict: "FAIL"` (`labelVerdictCorrect: true`);
  `class_type` resolves `RESOLVED_MISMATCH` (both artifacts agree).
- case-29: `cascadeVerdict.actualLabelVerdict: "FAIL"` (`labelVerdictCorrect: true`);
  `brand_name` resolves `RESOLVED_MISMATCH` (both artifacts agree).

The router stage alone still reads `REVIEW` / `AMBIGUOUS_BRAND` on both cases (unchanged,
matches the original diagnosis) — the resolver is what completes the correct `FAIL`. Per the
orchestrator's ruling, this is the "expectations correct as written" branch: no manifest edit,
no rubric change. `KNOWN_VECTOR_GAPS` stays empty. case-29 keeps its `V8` tag as that vector's
only carrier.
...
**Code fix found by this ticket's own tests: `applyDegradation`'s dispatcher,
`scripts/golden/degrade.ts`.** The `"low-light"` case built its `applyLowLight` params object
by hand, forwarding only `region` and `brightnessFactor`. A new test caught it before any live
run: a manifest entry naming `contrastFactor` / `noiseAmplitude` built byte-identical to one
without them. Fixed by forwarding both new keys. `src/lib/golden-set/loader.ts`'s
`DEGRADATION_PARAM_SHAPE` also gained the two new optional keys for `"low-light"` — the
loader's own closed-schema check rejected case-21's manifest entry outright until this landed.

**Tests.** `scripts/golden/degrade.test.ts`: 9 new cases for `contrastFactor` /
`noiseAmplitude` — byte-identical defaults, directional pixel checks, range rejection,
determinism, and one case that proves the dispatcher forwards the new params (red without the
dispatcher fix above). `scripts/golden/images.test.ts` and
`scripts/eval/warning-golden-cases.test.ts` updated to match the new manifest content they
pin (case-21's `degradations` array; case-23/24's `WARNING_MISMATCH`) — not weakened, both
still assert an exact value, now the corrected one. `pnpm test`: 1741/1741 pass. `pnpm
typecheck`, `pnpm lint`: clean. `pnpm golden:verify`: PASS, 32 cases checked, vector coverage
unchanged (`KNOWN_VECTOR_GAPS` still empty; V4 and V8 keep their existing carriers).

**The `verified` flag.** Every edited case — `case-21`, `case-23`, `case-24`, `case-25`,
`case-26` — keeps `verified: false`. This ticket is a machine edit. `golden-set/README.md:81-85`
reserves `verified: true` for a human sign-off. Troy: these five need review.

**Measured, full-corpus, real, live run — the closing evidence, `claude-haiku-4-5` /
`claude-sonnet-5`, `pnpm eval:check -- --live --full`, measured 2026-08-13T01:47:56.655Z, 32/32
cases scored, 0 failures, $0.27957.**
...
the extractor. `baseline.json` is promoted from this exact report (`ticket`, `measuredAt`,
`manifestVersion`, `manifestContentHash`, `caseIds`, `summary` — the identical field selection
`check.ts`'s own `--update-baseline` branch writes), not from a second live run: the
already-committed `eval-report.json` this run just produced already carries every field a
promotion needs, and re-spending ~$0.28 to regenerate an identical report would not be a more
honest number, only a more expensive one. `pnpm eval:check` (cheap mode) now passes against
this baseline.

**Attribution — checked case by case, not assumed.** Router-verdict accuracy's full +2 (24 to
26) is this ticket's own doing: `git show HEAD:scripts/eval/results/eval-report.json`'s
pre-ticket router-wrong set is exactly `{case-17, case-19, case-21, case-22, case-25, case-26,
case-28, case-29}` (8 cases). This run's router-wrong set is that same 8 minus `case-25` and
`case-26` — the two cases C1/C2 fix — and nothing else moved. Cascade-verdict accuracy's +4 (22
to 26) splits: +2 from the same two cases (case-25/26 never escalate, so router-correct means
cascade-correct too), and +2 from `case-16` and `case-19` — neither one this ticket edited.
Both flipped cascade-wrong to cascade-correct because their RESOLVER call returned a different
outcome this run (`case-16`: `resolved` in the pre-ticket run, `needs-human` here; `case-19`:
`needs-human` in the pre-ticket run, `resolved` here) — ordinary resolver-call variance on an
untouched case, the same kind of run-to-run model variance TRO-543 measures directly on
case-17. **Derived, not claimed as this ticket's fix:** the cascade-accuracy headline number
overstates what C1 through C8 changed by 2 of 32 cases. The router-accuracy number does not.

**Total measured spend, this ticket, all real `claude-haiku-4-5` / `claude-sonnet-5` API
calls: $0.33795.** Six targeted `--live --case=<id>` runs while verifying each correction
(case-25 $0.004675, case-26 $0.004795, case-23 $0.014633, case-24 $0.014886, case-21 modest
$0.004685, case-21 stronger-variant experiment $0.014706 — reverted, kept for the record above)
plus the one full-corpus run, $0.27957.

**Not verified by this ticket.** Whether Troy wants case-21's genuinely-degraded pixels to also
close S6 (confidence-based `LOW_IMAGE_QUALITY` routing has no deterministic signal) — a
separate, already-diagnosed defect this ticket does not fix. Whether the cascade-level
resolver regression TRO-538 first raised as an open question — a correct router `REVIEW`
flipped to an incorrect `PASS` — warrants a resolver-prompt change. TRO-538's own run named
four cases (case-16, case-18, case-23, case-24); this run reproduces three of the four
(case-18, case-23, case-24) — case-16 did not reproduce this time, per the Attribution note
above, itself a small piece of evidence that the regression is real but not always the same
size. This ticket answers nothing further about it.
...
--update-baseline` to re-measure and re-promote the whole corpus for real (~$0.28).

**Rollback.** `git revert` this ticket's commit(s). `golden-set/images/case-21-low-light-front-
label.jpg` reverts with the manifest change; `pnpm golden:build` after the revert regenerates
it to match. `scripts/eval/results/eval-report.json` and `scripts/eval/baseline.json` need a
fresh `--live --full --update-baseline` run to re-establish the pre-ticket numbers — both are
working artifacts, committed for evidence, not source, the same rollback shape TRO-535's own
entry above uses.
