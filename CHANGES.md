...
**What changed.** Replaced the USWDS-influenced visual language with a Notion-style look:
white page, warm near-black text (`#37352f`), thin soft borders, larger radius, generous
whitespace, one quiet accent blue. Dark mode is gone — no `prefers-color-scheme` branching, so
a user's own OS setting can no longer silently hand them a look nobody has reviewed for them.
Troy-directed, in-session. Pure CSS: `src/app/globals.css` is the only styled file touched, plus
a two-line doc update. No component markup, no component logic, no behavior change.
...
color itself is drawn: a thin neutral border plus one colored left-edge accent stripe (a
Notion-style callout), replacing the previous full-perimeter colored border.
...
white button text 5.10:1, the focus ring 3.88:1, and each verdict color against its own tint
background — match 4.65:1, mismatch 4.81:1, review 5.10:1. All measured with the real WCAG
relative-luminance formula (`src/lib/utils/contrast.ts`), not eyeballed from a screenshot.

**New regression test, not a G6 exception.** `src/app/globals-contrast.test.ts` parses the real
`:root` custom-property values out of `globals.css`. It asserts every critical pair against its
WCAG floor — a permanent guard against a future palette edit, not a one-time check. It also
asserts the file carries no `prefers-color-scheme: dark` block. That assertion was false before
this ticket's change: the file still had a dark-mode block, so the test failed for that exact
reason before the CSS edit, and passed after — red-first, confirmed directly, not asserted after
the fact. `src/lib/utils/contrast.ts`'s own unit tests (`contrast.test.ts`) check the WCAG math
itself against a known reference value: WCAG's own worked example, `#767676` on white, is
4.54:1.

**Live-verified, not just measured on paper.** Walked every screen in a real Chromium session
against this worktree's own `pnpm dev` (access-code, verify empty/error/detail ×3, review queue
list/detail, batch upload/results) and ran axe-core (Deque's WCAG rule engine) against all
eight: **0 violations**, matching TRO-570's own clean baseline — this reskin did not regress it.

**`docs/PRD.md` §5 updated** so the aesthetic line stays accurate for the other sessions working
this repo concurrently — it now names Notion-style as the settled direction and states plainly
that it supersedes the prior USWDS-influenced line.
...
`globals.css` values, not estimated); the 0-violation axe-core result across all 8 screens
(real Chromium, this worktree's own `pnpm dev`); the red-before/green-after state of the new
test file (run directly against `globals.css` before and after the CSS edit).
**Not verified:** how this reads on a real screen-reader (VoiceOver/NVDA) — no reason to expect
a change from TRO-570's own structural findings (headings/landmarks/alt text/`role="alert"` are
all untouched by this ticket), but not independently re-walked here since this ticket is
CSS-only and none of those structural elements changed.

### Rollback

`git revert` this ticket's commit. One commit, three new/changed files
(`globals.css`, `src/lib/utils/contrast.ts` + test, `globals-contrast.test.ts`) plus the
`docs/PRD.md` line — no schema change, no migration, nothing to undo outside the app source tree
and one doc line.
