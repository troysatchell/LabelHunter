...
**TH-R2 dropped VERIFIED → PARTIAL. The reason is honest, not regressive.** The latency artifact measures a pipeline that no longer ships. `scripts/latency/results/single-label-verify.json` was measured 2026-08-12T02:17:14Z. The warning comparator was wired into the route at commit c5e49f8, roughly an hour later. The artifact's own `pipelineScope` field said so at the time: *"No OCR/warning-subsystem comparator."* All 20 of its runs returned REVIEW / LOW_MODEL_CONFIDENCE — never a clean pass. The committed artifact's real figure is p50 **3690 ms**, p95 4339 ms (`single-label-verify.json:30-31`). An earlier draft of this line quoted 4232 ms instead. That figure belongs to a DIFFERENT, superseded artifact (commit `bce6b71`, replaced the same day by `5a16263`). It was never the committed file's own number. Corrected here, per TRO-539. Both the 3690 ms run and the superseded 4232 ms run measure the same wrong thing: neither includes the warning comparator. TRO-519 has since landed a 2000ms OCR deadline. That closes the "p95 is unbounded" gap this paragraph used to name. But the committed artifact still predates that fix too. No re-measurement against the shipping pipeline exists yet. TH-R2 stays PARTIAL until one does.
