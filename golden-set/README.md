...
`rendered+degraded` case. That is 31 of 31 cases. Total size: about 1.18 MB. Largest file:
...
of ≥20), not any single case. The manifest's 31 cases satisfy the ≥20 count, but no case
...
warning (3), and reworded warning (3). Every other category has 2, except tiny warning text,
which has 1: TRO-516 C5 merged `case-24-tiny-warning-text-miniature-bottle` into
...
Clean match is the largest group because it also carries every format-variant vector (V1, V6,
V7) that needs a fully-matching label to isolate cleanly.
