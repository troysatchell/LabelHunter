...
const MANIFEST_COLUMN_SET: ReadonlySet<string> = new Set(MANIFEST_COLUMNS);

function isManifestColumn(name: string): name is ManifestColumn {
  return MANIFEST_COLUMN_SET.has(name);
}
...
export function buildManifestCsv(rows: ManifestCsvRow[], overrideHeader?: readonly string[]): string {
  ...
  const lines = rows.map((row) => {
    ...
    return columns.map((col) => csvField(cells[col as ManifestColumn])).join(",");
  });
