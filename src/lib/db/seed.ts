...
async function main() {
  ...
  try {
    await db.transaction(async (tx) => {
      ...
      const [appMillerVineyards] = await tx
        .insert(schema.applications)
        .values({
          batchJobId: batch.id,
          beverageType: "wine",
      ...
      const [imgMillerVineyards] = await tx
        .insert(schema.labelImages)
        .values({
          batchJobId: batch.id,
      ...
      const [verMillerVineyards] = await tx
        .insert(schema.verifications)
        .values({
      ...
      await tx.insert(schema.fieldResults).values([
